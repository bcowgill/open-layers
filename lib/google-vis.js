...
var oMap, oActiveLayer, oHeatMapLayer, oCentroidLayer, oStaticIpLayer,
	startZoom = 8, // was 13
...
function getStaticIpData(aStaticIpLatLon, minRadius, maxRadius)
{
	var aPoints = [], radius, theMaxRadius =  0,
		LAT = 1, LON = 2, RADIUS = 3;
	aStaticIpLatLon.forEach(function (aIpLatLonRadius) {
		if (aIpLatLonRadius.length)
		{
			radius = aIpLatLonRadius[RADIUS];
			...
			{
				aPoints.push(new google.maps.LatLng(aIpLatLonRadius[LAT], aIpLatLonRadius[LON]));
			}
		}
	});
	console.log(" Total Data points", aPoints.length);
	console.log("getStaticIpData min/max range, max radius found, points", minRadius, maxRadius, theMaxRadius, aPoints.length);
	return aPoints;
}
...
function initialize() {
	...
	oStaticIpLayer = new google.maps.visualization.HeatmapLayer({
		data: new google.maps.MVCArray(getStaticIpData(StaticIPLatLonRadiusMeters, MIN_RADIUS, MAX_RADIUS))
	});

	oStaticIpLayer.set('radius', RADIUS);
	oStaticIpLayer.set('opacity', OPACITY / 100);

	oActiveLayer = oStaticIpLayer;
	oActiveLayer.setMap(oMap);
}

function toggleHeatmap() {
	if (oActiveLayer === oHeatMapLayer)
	{
		console.log("toggleHeatMap from HeatMap to Centroid");
		oActiveLayer.setMap(null);
		oActiveLayer = oCentroidLayer;
		oActiveLayer.setMap(oMap);
	}
	else if (oActiveLayer === oCentroidLayer)
	{
		console.log("toggleHeatMap from Centroid to Static Ip");
		oActiveLayer.setMap(null);
		oActiveLayer = oStaticIpLayer;
		oActiveLayer.setMap(oMap);
	}
	else if (oActiveLayer === oStaticIpLayer)
	{
		console.log("toggleHeatMap from Static Ip to HeatMap");
		oActiveLayer.setMap(null);
		oActiveLayer = oHeatMapLayer;
		oActiveLayer.setMap(oMap);
	}
	else
	{
		console.log("toggleHeatMap from Unknown to HeatMap");
		oActiveLayer.setMap(null);
		oActiveLayer = oHeatMapLayer;
		oActiveLayer.setMap(oMap);
	}
